# 次期アップデート計画 (v1.1.0) - 通信量最 1. **曲の時間取得**: 現在再生中の曲の総時間を取得

2. **動的ポーリング間隔**: 曲の残り時間に基づく最適化
   - 曲の残り時間が 3 秒以内の場合のみ確認（曲終了が近い場合）
   - 曲終了後 3 秒以上経過してから次の確認を行う（切り替わりを確実に検知）
   - 曲切り替わり直後は 3 秒待機してから通常間隔（15 秒）に戻す
   - 同じ曲が長時間（45 秒以上）再生されている場合は長期間隔（60 秒）に切り替え**最終更新**: 2025 年 9 月 25 日
...
2. **動的ポーリング間隔**: 曲の残り時間に基づく最適化
   - 曲の残り時間が 30 秒以内の場合：曲終了後 3 秒以上待ってから次の確認を行う
   - 同じ曲が長時間（45 秒以上）再生されている場合：長期間隔（60 秒）に切り替え
   - それ以外：通常間隔（15 秒）で確認を継続
...
ポーリング時:
  if (曲の残り時間が30秒以内) {
    // 曲終了を予測して、終了後3秒待ってから次の確認を行う
    曲終了待機フラグを設定
    次回のポーリングまで: 残り時間 + 3秒
  } else if (同じ曲が45秒以上再生中) {
    // 長期間隔で確認を続ける
    次の曲情報を取得
    人気度を表示
    次回のポーリングまで: 60秒
